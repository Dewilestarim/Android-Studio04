...
class ListProfileAdapter :RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class Holder(val binding: ItemProfileBinding) :RecyclerView.ViewHolder(binding.root){
        ...
        //membuat fungsi untuk menghubungkan binding dan view (item_profile.xml)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        //menginisiasi class viewholder yag telah dibuat sebelum (holder)
        when(viewType){
            0 -> { val binding = ItemProfileHeaderBinding.inflate(layoutInflater,parent,false)
                return HolderHeader(binding)
            }
            1 -> { val binding = ItemProfileBinding.inflate(layoutInflater,parent,false)
